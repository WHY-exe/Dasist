...
namespace DCBuf
{
	...
	size_t LayoutElement::GetOffsetEnd() const noexcept
	{
		switch (m_type)
		{
		#define X(el) case el: return *m_offset + Map<el>::hlsl_size;
			LEAF_ELEMENT_TYPES;
		#undef X
		case Struct:
		{
		...
		{
			const auto& data = static_cast<ExtraData::Array&>(*m_pExtraData);
			return *m_offset + AdvanceToBoundary(data.m_layoutElement->GetSizeInBytes()) * data.m_size;
		}
		default:
	...
	LayoutElement& LayoutElement::Add( const std::string& name, DataType dt) noexcept(!IS_DEBUG)
	{
		...
		auto& struct_data = static_cast<ExtraData::Struct&>(*m_pExtraData).m_layoutElements;
	...
	size_t LayoutElement::Finalize(size_t offsetIn) noexcept(!IS_DEBUG)
	{
		...
		{
		#define X(el)\
			case el: m_offset =  AdvanceIfCrossBoundary(offsetIn, Map<el>::hlsl_size); return *m_offset + Map<el>::hlsl_size;
			LEAF_ELEMENT_TYPES
		#undef X
		case Struct:
	...
	std::string LayoutElement::GetSignatureForStruct() const noexcept
	{
		...
		for (auto& el : data)
		{
			result += "Name : " + el.first  + ", Signature" + el.second.GetSignature() + ";\n";
		}
		result += "}";
		return result;
	}
	...
	std::string LayoutElement::GetSignatureForArray() const noexcept
	{
		...
		auto& data = static_cast<ExtraData::Array&>(*m_pExtraData);
		result += "Size : " + std::to_string(data.m_size) + ", DataType : " + data.m_layoutElement->GetSignature() + "};";
		return result;
	}
	// in gpu side's const buffer, data align in 16Byte
	// this functoin allow us to advance to the  offset at the begining of of next block of data
	...
	size_t LayoutElement::AdvanceIfCrossBoundary(size_t offset, size_t size) noexcept
	{
	...
	ConstElementRef::ConstElementRef(size_t offset, const char* pByte, const LayoutElement* playout)
		:
		m_offset(offset),
		m_pByte(pByte),
		m_pLayout(playout)
	...
	ElementRef::ElementRef(size_t offset, char* pByte, const LayoutElement* playout) noexcept
		:
		m_offset(offset),
		m_pByte(pByte),
		m_pLayout(playout)
