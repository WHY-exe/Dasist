...
{
	m_PSCbuf.Update(gfx, ModelCBuffer(m_Ambient, 1.0f, 30.0f));
	m_PSCbuf.Bind(gfx);
}
...
void ModelObj::SetPos(float x, float y, float z) noexcept
{
	m_pos = { x, y, z };
}

void ModelObj::SetPos(DirectX::XMFLOAT3 pos) noexcept
{
	m_pos = pos;
}

void ModelObj::Scale(float scale) noexcept
{
	m_Scale = scale;
}
