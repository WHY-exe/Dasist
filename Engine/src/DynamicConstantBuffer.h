...
#undef X
	...
	public:
		...
		size_t Resolve() const noexcept(!_DEBUG)
		{
			...
			{
			#define X(el)\
			case el: assert(typeid(Map<el>::SysType) == typeid(T)); return *m_offset;
				LEAF_ELEMENT_TYPES
			#undef X
			default:
	...
	private:
		ConstElementRef(size_t offset, const char* pByte, const LayoutElement* playout);
		size_t m_offset;
		const char* m_pByte;
	...
	private:
		ElementRef(size_t offsett, char* pByte, const LayoutElement* playout) noexcept;
		size_t m_offset;
		const LayoutElement* m_pLayout;
		char* m_pByte;
	};
