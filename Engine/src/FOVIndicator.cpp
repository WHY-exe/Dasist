...
FOVIndicator::FOVIndicator(Graphics& gfx, float width, float height, float nearZ, float farZ)
{
	...
	AddEssentialBind(Topology::Resolve(gfx, D3D11_PRIMITIVE_TOPOLOGY_LINELIST));
	{
		Technique line("line_draw");
		{
			...
			non_occluded.AddBind(PixelShader::Resolve(gfx, L"res\\cso\\Solid_PS.cso"));
			struct PSColorConstant
			{
				DirectX::XMFLOAT3 color = { 0.6f,0.2f,0.2f };
				float padding = 0.0f;
			} colorConst;
			non_occluded.AddBind(PixelConstantBuffer<PSColorConstant>::Resolve(gfx, colorConst, 3u));
		...
		}
		{
			...
			occluded.AddBind(PixelConstantBuffer<PSColorConstant>::Resolve(gfx, colorConst, 3u));
...
void FOVIndicator::SetVertices(Graphics& gfx, float width, float height, float nearZ, float farZ)
{
	Vertex::Layout layout;
	layout.Append(Vertex::Position3D);
...
void FOVIndicator::SetRot(DirectX::XMFLOAT3 rot) noexcept
{
	m_rotation = rot;
}

void FOVIndicator::SetScale(float scale) noexcept
{
	m_scale = scale;
}
