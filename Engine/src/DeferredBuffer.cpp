...
void DeferredBuffer::BindAsBuffer(Graphics& gfx) noexcept(!IS_DEBUG)
{
    IMPORT_INFOMAN(gfx);
    GFX_THROW_INFO_ONLY(
        GetContext(gfx)->OMSetRenderTargets(buffer_num, m_RTVs->GetAddressOf(), m_DSV.Get())
    );
    // configure viewport
    D3D11_VIEWPORT vp = {};
...
void DeferredBuffer::Clear(Graphics& gfx, std::array<float, 4> color) noexcept(!IS_DEBUG)
{
    IMPORT_INFOMAN(gfx);
    ...
    GetContext(gfx)->ClearDepthStencilView(m_DSV.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0u);
}
    ...
    m_height(height)
{
    ...
    {
        GFX_THROW_INFO(
            GetDevice(gfx)->CreateRenderTargetView(pTexture.Get(), &rtvDesc, &m_RTVs[i])
        );
    }
    ...
    GFX_THROW_INFO(GetDevice(gfx)->CreateDepthStencilView(
       pDepthStencilTex.Get(), &descView, &m_DSV
    ));
}
...
DeferredBufferAsShaderInput::DeferredBufferAsShaderInput(Graphics& gfx, UINT width, UINT height, UINT slot)
    :
    ...
    m_slot(slot)
{
    ...
    for(size_t i = 0; i < buffer_num; i++)
        GFX_THROW_INFO(GetDevice(gfx)->CreateShaderResourceView(pTexture.Get(), &srvDesc, &m_PSRV[i]));
}

DeferredBufferAsShaderInput::DeferredBufferAsShaderInput(Graphics& gfx, UINT slot)
    :
    DeferredBufferAsShaderInput(gfx, gfx.GetWindowWidth(), gfx.GetWindowHeight(), slot)
{
}
...
void DeferredBufferAsShaderInput::Bind(Graphics& gfx) noexcept(!IS_DEBUG)
{
    IMPORT_INFOMAN(gfx);
    GFX_THROW_INFO_ONLY(
        GetContext(gfx)->PSSetShaderResources(m_slot, buffer_num, m_PSRV->GetAddressOf())
    );
}

void DeferredBufferAsShaderInput::Resize(Graphics& gfx, UINT width, UINT height)
{
    *this = DeferredBufferAsShaderInput(gfx, width, height);
}

DeferredBufferAsTarget::DeferredBufferAsTarget(Graphics& gfx, UINT width, UINT height)
    :
    DeferredBuffer(gfx, width, height)
{
}
