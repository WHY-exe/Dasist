...
std::vector<UINT>& MeshData::GetIndicies() noexcept
{
	return m_indicies;
}

DCBuf::Buffer& MeshData::GetConstData() noexcept
{
	return m_DynamicConstData;
}

const std::wstring& MeshData::GetPSPath() const noexcept
{
	return m_szPSPath;
}

const std::wstring& MeshData::GetVSPath() const noexcept
{
	return m_szVSPath;
}
	...
	for (unsigned int i = 0; i < mesh.mNumVertices; i++)
	{
		for (size_t i_ele = 0; i_ele < vlayout.Count(); i_ele++)
		{
			...
			case Vertex::Normal:
				...
				break;
			case Vertex::Tex2D:
				m_pVertexData->EmplaceBack(*reinterpret_cast<DirectX::XMFLOAT2*>(&mesh.mTextureCoords[0][i]));
				break;
			case Vertex::Tangent:
				m_pVertexData->EmplaceBack(*reinterpret_cast<DirectX::XMFLOAT3*>(&mesh.mTangents[i]));
				break;
			case Vertex::Bitangent:
				m_pVertexData->EmplaceBack(*reinterpret_cast<DirectX::XMFLOAT3*>(&mesh.mBitangents[i]));
				break;
			default:
...
void MeshData::SetMaterial(Graphics& gfx, const aiMesh& mesh, const aiMaterial* const* pMaterial)
{
	...
	if (mesh.mMaterialIndex >= 0)
	{
		...
		{
			m_texInfo.m_amTex = Texture::Resolve(gfx, ANSI_TO_UTF8_STR(szTexRootPath + "\\"s + texPath.C_Str()), 3);
		...
		{
			m_texInfo.m_difTex = Texture::Resolve(gfx, ANSI_TO_UTF8_STR(szTexRootPath + "\\"s + texPath.C_Str()));
		...
		{
			m_texInfo.m_specTex = Texture::Resolve(gfx, ANSI_TO_UTF8_STR(szTexRootPath + "\\"s + texPath.C_Str()), 1);
		...
		if (material.GetTexture(aiTextureType_NORMALS, 0, &texPath) == aiReturn_SUCCESS || 
			material.GetTexture(aiTextureType_DISPLACEMENT, 0, &texPath) == aiReturn_SUCCESS && 
			texPath.length != 0)
		{
