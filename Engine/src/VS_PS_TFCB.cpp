...
VS_PS_TFCB::VS_PS_TFCB(Graphics& gfx, const Drawable& parent, UINT slotV, UINT slotP)
	:
...
void VS_PS_TFCB::Bind(Graphics& gfx) noexcept
{
...
TransformCbuf::Transform VS_PS_TFCB::GetTransform(Graphics& gfx) const noexcept
{
	return TransformCbuf::GetTransform(gfx);
}

void VS_PS_TFCB::UpdateBindImpl(Graphics& gfx, const Transform& tf) const noexcept
{
