...
{
public:
	static IndexedTriangleList MakeTesselated( Vertex::Layout layout,int latDiv,int longDiv )
	{
		...
		Vertex::DataBuffer vb{ std::move( layout ) };
		...
		return IndexedTriangleList{ std::move( vb ),std::move( indices ) };
	}
	static IndexedTriangleList Make( std::optional<Vertex::Layout> layout = std::nullopt )
	{
		if( !layout )
		{
			layout = Vertex::Layout{}.Append( Vertex::Position3D );
		}
