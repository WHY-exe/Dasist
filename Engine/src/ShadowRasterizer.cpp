...
void ShadowRasterizer::ResetShadowBiasParams(Graphics& gfx, int depthBias, float slopeBias, float clamp) noexcept(!IS_DEBUG)
{
...
int ShadowRasterizer::GetDepthBias() const noexcept
{
	return m_depthBias;
}

float ShadowRasterizer::GetSlopeBias() const noexcept
{
	return m_slopeBias;
}

float ShadowRasterizer::GetClamp() const noexcept
{
	return m_clamp;
}
